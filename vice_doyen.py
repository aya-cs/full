...
def vice_doyen_dashboard():
    ...
    if page == "🏠 Vue Globale & KPIs":
        ...
        with c1: kpi_card("📝 Examens planifiés", int(total_examens), "Planifie / Confirme", "ok")
        with c2: kpi_card("🏢 Utilisation salles", f"{float(taux_salles):.1f}%", "Occupation", "warn" if float(taux_salles) < 40 else "ok")
        with c3: kpi_card("⚠️ Conflits", int(conflits), "Doit être à 0", "danger" if int(conflits) > 0 else "ok")
        with c4: kpi_card("✅ Taux confirmés", f"{float(taux_confirmes):.1f}%", "Objectif ≥ 60%", "warn" if float(taux_confirmes) < 60 else "ok")
    ...
    # =========================================================
        ...
        if conflits is None or conflits.empty:
            st.success("🎉 Aucun conflit détecté.")
        else:
            st.dataframe(conflits, use_container_width=True)
    ...
    # =========================================================
        ...
        if st.button("🔍 Lancer l'analyse", type="primary"):
            conflits = detecter_tous_les_conflits()
            st.session_state["vd_conflits"] = conflits

        conflits = st.session_state.get("vd_conflits", pd.DataFrame())
    ...
    # =========================================================
        ...
        with c1: kpi_card("🧾 Total", len(planning), "", "ok")
        with c2: kpi_card("⏳ Planifiés", planifies, "À confirmer", "warn" if planifies > 0 else "ok")
        with c3: kpi_card("✅ Confirmés", confirmes, "Validés", "ok")
